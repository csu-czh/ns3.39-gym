...
bool
Session::isPositiveLink(int a, int b)
{
...
Session::getBFIndex(Mnode* node)
{
    ...
    else if ((node->type.compare("aggr") == 0 || node->type.compare("edge") == 0) &&
             node->pod == sender->pod)
    {
        return 0;
    }
...
void
dijkstra(int st, int dist[], Topolopy* topolopy)
{
    int n = topolopy->nodes.size();
    bool visited[n];
    for (int i = 0; i < n; i++)
    ...
    {                   // 每次循环都会剔除掉1个点，因此需要for循环遍历n次。
        ...
        { // find the index of min distance
            ...
            { // 当前的点没有被踢出，并且当前点的距离比index点的距离小，则更新index。index ==
              // -1表示还未开始找到dist中最小值，则把dist[1]加入。
                index = j;
            }
        }
