...
#include "ipv4-orca-routing-helper.h"
...
NS_LOG_COMPONENT_DEFINE("Ipv4OrcaRoutingHelper");

Ipv4OrcaRoutingHelper::Ipv4OrcaRoutingHelper()
{
}

Ipv4OrcaRoutingHelper::Ipv4OrcaRoutingHelper(const Ipv4OrcaRoutingHelper& o)
{
}

Ipv4OrcaRoutingHelper*
Ipv4OrcaRoutingHelper::Copy() const
{
    return new Ipv4OrcaRoutingHelper(*this);
}

Ptr<Ipv4RoutingProtocol>
Ipv4OrcaRoutingHelper::Create(Ptr<Node> node) const
{
    return CreateObject<Ipv4OrcaRouting>();
}

Ptr<Ipv4OrcaRouting>
Ipv4OrcaRoutingHelper::GetOrcaRouting(Ptr<Ipv4> ipv4) const
{
    NS_LOG_FUNCTION(this);
    Ptr<Ipv4RoutingProtocol> ipv4rp = ipv4->GetRoutingProtocol();
    NS_ASSERT_MSG(ipv4rp, "No routing protocol associated with Ipv4");
    if (DynamicCast<Ipv4OrcaRouting>(ipv4rp))
    {
        NS_LOG_LOGIC("Static routing found as the main IPv4 routing protocol.");
        return DynamicCast<Ipv4OrcaRouting>(ipv4rp);
    }
    if (DynamicCast<Ipv4ListRouting>(ipv4rp))
    {
        ...
        for (uint32_t i = 0; i < lrp->GetNRoutingProtocols(); i++)
        {
            ...
            if (DynamicCast<Ipv4OrcaRouting>(temp))
            {
                NS_LOG_LOGIC("Found static routing in list");
                return DynamicCast<Ipv4OrcaRouting>(temp);
            }
        }
    }
...
void
Ipv4OrcaRoutingHelper::AddMulticastRoute(Ptr<Node> n,
                                           Ipv4Address source,
                                           Ipv4Address group,
                                           Ptr<NetDevice> input,
                                           NetDeviceContainer output)
{
    ...
    for (NetDeviceContainer::Iterator i = output.Begin(); i != output.End(); ++i)
    {
        ...
        NS_ASSERT_MSG(interface >= 0,
                      "Ipv4OrcaRoutingHelper::AddMulticastRoute(): "
    ...
    Ipv4OrcaRoutingHelper helper;
    Ptr<Ipv4OrcaRouting> ipv4StaticRouting = helper.GetOrcaRouting(ipv4);
    if (!ipv4StaticRouting)
    {
        NS_ASSERT_MSG(ipv4StaticRouting,
                      "Ipv4OrcaRoutingHelper::SetDefaultMulticastRoute(): "
                      "Expected an Ipv4OrcaRouting associated with this node");
    }
...
void
Ipv4OrcaRoutingHelper::AddMulticastRoute(std::string nName,
                                           Ipv4Address source,
                                           Ipv4Address group,
                                           Ptr<NetDevice> input,
                                           NetDeviceContainer output)
...
void
Ipv4OrcaRoutingHelper::SetDefaultMulticastRoute(Ptr<Node> n, Ptr<NetDevice> nd)
{
    Ptr<Ipv4> ipv4 = n->GetObject<Ipv4>();
...
void
Ipv4OrcaRoutingHelper::SetDefaultMulticastRoute(Ptr<Node> n, std::string ndName)
{
...
void
Ipv4OrcaRoutingHelper::SetDefaultMulticastRoute(std::string nName, Ptr<NetDevice> nd)
{
    Ptr<Node> n = Names::Find<Node>(nName);
    SetDefaultMulticastRoute(n, nd);
}
...
void
Ipv4OrcaRoutingHelper::SetDefaultMulticastRoute(std::string nName, std::string ndName)
{
    Ptr<Node> n = Names::Find<Node>(nName);
    Ptr<NetDevice> nd = Names::Find<NetDevice>(ndName);
    SetDefaultMulticastRoute(n, nd);
