...
main(int argc, char* argv[])
{
    ...
    staticRoutingA->AddHostRouteTo(Ipv4Address("10.1.6.1"), Ipv4Address("10.1.1.2"), 1);
    ...
    staticRoutingB->AddHostRouteTo(Ipv4Address("10.1.6.1"), Ipv4Address("10.1.6.1"), 6);
    Ptr<Ipv4StaticRouting> staticRoutingC = ipv4RoutingHelper.GetStaticRouting(c.Get(0)->GetObject<Ipv4>());
    staticRoutingC->AddHostRouteTo(Ipv4Address("10.1.4.1"), Ipv4Address("10.1.3.2"), 1);
    ...
    // UdpClientHelper client2 ( interLine6.GetAddress(0), 9);
    // client.SetAttribute ("MaxPackets", UintegerValue (300));
    // client.SetAttribute ("Interval", TimeValue (Seconds (0.05)));
    // client.SetAttribute ("PacketSize", UintegerValue (1024));

    // ApplicationContainer apps2 = client2.Install (c.Get(1));
